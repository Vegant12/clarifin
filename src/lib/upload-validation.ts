/**
 * Server-side PDF upload validator (INFRA-01 / D-24).
 *
 * Phase 1 ships the helper and tests. Phase 2 wires `validatePdfUpload` into
 * the `/api/upload-init` route handler — the call MUST happen before any
 * Supabase Storage operation (signed URL issuance, direct upload, etc.).
 *
 * The 50 MB limit applies to the raw PDF size as reported by the browser's
 * `File.size`. Magic-byte verification (`validatePdfMagicBytes`) is a
 * defense-in-depth helper Phase 2 calls AFTER upload to confirm the file
 * actually starts with %PDF- (the browser-reported `type` is user-controllable
 * and cannot be fully trusted).
 */

export const MAX_PDF_BYTES = 50 * 1024 * 1024; // 50 MB = 52428800 bytes
...
export function validatePdfUpload(
  ...
  if (input.size > MAX_PDF_BYTES) {
    ...
    return {
      ok: false,
      reason: `File is ${mb} MB. Maximum allowed is 50 MB.`,
    };
  }
  ...
  const bytes =
    buffer instanceof Uint8Array
      ...
      : new Uint8Array(
          buffer,
          0,
          Math.min(PDF_MAGIC_BYTES.length, buffer.byteLength),
        );
