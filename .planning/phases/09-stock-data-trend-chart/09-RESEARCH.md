...
1. **yahoo-finance2 `incomeStatementHistory` availability for major IDX tickers**
   - What we know: The module exists and is documented. IDX coverage is expected to be good for large-caps (BBCA, TLKM, BBRI).
   - What's unclear: Whether `totalRevenue` / `netIncome` are the correct field names vs. `grossProfit` / `ebit` etc.
   - Recommendation: In Wave 0, run a live test call `yahooFinance.quoteSummary('BBCA.JK', { modules: ['incomeStatementHistory'] })` and `console.log(JSON.stringify(..., null, 2))` to confirm exact field names before writing the transformer.

2. **Cache placement: new columns vs. new `stock_cache` table**
   - What we know: D-07 recommends new columns on `documents`; this avoids a join.
   - What's unclear: Planner may prefer a separate table for clean separation.
   - Recommendation: Use new columns on `documents` (fewer migrations, simpler RSC fetch). If `stock_data` JSON grows large (unlikely), revisit.

3. **Stock data fetch location: RSC vs. dedicated API route**
   - What we know: D-06 defines `GET /api/stock/[ticker]` as the route. D-08 says ExplanationPanel gets props from the RSC.
   - What's unclear: Does `page.tsx` RSC call `fetchStockData` directly (inline, returns cached data), or does it call the `/api/stock/[ticker]` route via fetch?
   - Recommendation: RSC calls `fetchStockData()` directly (server function to server function; no HTTP round-trip needed). The `/api/stock/[ticker]` route exists for potential client-side refresh or external callers. Both can coexist.
