...
describe("ExplanationPanel", () => {
  ...
  it("Test 1: renders all 5 section headings in order", () => {
    render(<ExplanationPanel documentId="doc-1" explanation={fixture} score={null} onGoToPage={vi.fn()} ticker={null} stockData={null} chartData={null} stockError={false} />);
  ...
  it("Test 5: jargon detection is case-insensitive (ROE matches roe in dictionary)", () => {
    ...
    render(
      <ExplanationPanel documentId="doc-1" explanation={fixtureWithROE} score={null} onGoToPage={vi.fn()} ticker={null} stockData={null} chartData={null} stockError={false} />,
    );
    const jargonSpans = document.querySelectorAll(".decoration-dotted");
    expect(jargonSpans.length).toBeGreaterThan(0);
  });
  ...
  it("Test 6: clicking a citation calls onGoToPage with the correct page number", () => {
    const handle = vi.fn();
    render(<ExplanationPanel documentId="doc-1" explanation={fixture} score={null} onGoToPage={handle} ticker={null} stockData={null} chartData={null} stockError={false} />);
    const citation = screen.getByRole("button", { name: /view source for page 5/i });
  ...
  it("Test 7: snapshot — citations and jargon interleaved with plain text", () => {
    ...
    const { container } = render(
      <ExplanationPanel documentId="doc-snap" explanation={miniFixture} score={null} onGoToPage={vi.fn()} ticker={null} stockData={null} chartData={null} stockError={false} />,
    );
